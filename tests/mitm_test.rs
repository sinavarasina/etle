...
async fn mitm_with_wrong_psk_is_rejected() {
    ...
    let client_psk = AuthPsk::from_passphrase("kata sandi benar");
    let server_psk = AuthPsk::from_passphrase("kata sandi salah");
    ...
    assert!(client_result.is_err(), "client harus ditolak");
    assert!(server_result.is_err(), "server harus ditolak");
}
