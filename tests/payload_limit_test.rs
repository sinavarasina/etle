...
async fn codec_accepts_frame_at_exact_max_size() {
    ...
    send(&mut writer, &message).await.expect("send harus sukses pada batas MAX");

    let received = receive(&mut reader).await.expect("receive harus sukses pada batas MAX");
...
async fn codec_rejects_frame_one_byte_over_max() {
    ...
    let err = receive(&mut reader).await.expect_err("harus error untuk frame terlalu besar");
...
async fn codec_rejects_zero_length_frame() {
    ...
    let err = receive(&mut reader).await.expect_err("harus error untuk frame kosong");
...
async fn codec_rejects_u32_max_frame_length() {
    ...
    let err = receive(&mut reader).await.expect_err("harus error untuk u32::MAX");
...
fn chunker_accepts_minimum_chunk_size_of_one() {
    ...
    println!("[payload-limit] chunk_size=1: DITERIMA, {} chunks ✓", chunks.len());
    fs::remove_file(path).unwrap();
}
...
fn chunker_single_chunk_when_file_smaller_than_chunk_size() {
    ...
    println!(
        "[payload-limit] file 4 bytes dengan chunk_size 1MB: 1 chunk ✓"
    );
    fs::remove_file(path).unwrap();
}
...
fn chunker_default_chunk_size_is_valid() {
    ...
    assert_eq!(chunks.len(), 2, "data sedikit di atas 1MB harus menjadi 2 chunk");
...
async fn codec_handles_long_error_message() {
    ...
    let message = WireMessage::Error { message: long_msg.clone() };
