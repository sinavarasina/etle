...
fn single_bit_flip_causes_large_ciphertext_change() {
    ...
    let shared_secret = keypair_a
        .diffie_hellman(keypair_b.public_key())
        .unwrap();

    let file_key_a = derive_file_key(
        shared_secret.clone(),
        hash_file(&path_a).unwrap(),
    );

    let file_key_b = derive_file_key(
        shared_secret,
        hash_file(&path_b).unwrap(),
    );
    ...
    println!("[avalanche] 1-bit flip => {differing} differing bits dari {total_bits} ({:.1}%)", diff_ratio * 100.0);
...
fn file_id_avalanche_on_single_bit_flip() {
    ...
    for bit in 0..sample_bits {
        ...
        println!("[avalanche] bit {bit}: {diff}/256 bit FileId berubah ({:.1}%)", diff as f64 / 256.0 * 100.0);
    }

    let avg_diff = total_diff_bits as f64 / (sample_bits * 256) as f64;
    println!("[avalanche] rata-rata perubahan FileId: {:.1}%", avg_diff * 100.0);
...
fn manifest_hash_field_avalanche() {
    ...
    assert!(diff > 0, "Manifest tidak berubah sama sekali setelah modifikasi!");
}