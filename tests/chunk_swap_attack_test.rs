...
fn detects_chunk_from_different_file_replay() {
    ...
    let shared_secret = kp
        .diffie_hellman(kp2.public_key())
        .unwrap();
    let key_a = derive_file_key(shared_secret.clone(), hash_file(&path_a).unwrap());
    ...
    if let (Some(chunk_a_meta), Some(chunk_b_meta)) = (
        enc_a.manifest.chunks.first(),
        enc_b.manifest.chunks.first(),
    ) {
        assert_ne!(
...
fn join_chunks_is_order_agnostic_by_index() {
    ...
    assert_eq!(result, data.as_slice(), "join_chunks harus tetap benar meski urutan terbalik");
...
fn encrypted_chunks_roundtrip_is_tamper_evident() {
    ...
    assert_eq!(decrypted, data, "dekripsi dari file yang tidak dimodifikasi harus identik");
